...
::: {.column width="50%"}
![](assets/robot-pictures/predpod.png){fig-align="center" width="380"}
:::
::::::
...
<a class="btn btn-primary" download href="data/malawi.dta"> Malawi.dta </a>
...
#| eval: true
#| output: asis
cat("<details><summary>Click to view all variable labels</summary>\n<pre>\n")
for(i in seq_along(variable_labels)) {
  cat(names(variable_labels)[i], "\n")
  cat(variable_labels[i], "\n\n")
}
cat("</pre>\n</details>")
```
...
$1$ = the household IS poor <br> $0$ = the household is NOT poor

R's `dummyVars()` with fullRank = TRUE creates binary (0/1) dummy variables where:

The variable name format is: `original_variable.category_name` <br> $1$ means that observation belongs to that category<br> $0$ means it doesn't belong to that category <br> The first category is dropped to avoid multicollinearity(that's why we don't see poor.NotPoor or similar)
:::
...
Recall that the residual is estimated as the true (observed) value minus the predicted value. Thus: the max(imum) error of `{r} format(round(max_residual_r, 0), scientific=FALSE, big.mark=",")` suggests that the model under-predicted expenditure by this amount for at least one observation. From the estimation of the predicted residuals we obtain the popular measure of performance evaluation known as the Root Mean Squared Error (RMSE, for short).
...
The RMSE (`{r} format(round(rmse_insample_caret, 0), scientific=FALSE, big.mark=",")`) gives us an absolute number that indicates how much our predicted values deviate from the true (observed) number. This is all in reference to the target variable. Think of the question: How far, on average, are the residuals away from zero? Generally speaking, the lower the value, the better the model fit. Besides being a good measure of goodness of fit, the RMSE is also useful for comparing the ability of our model to make predictions on different (e.g. test) data sets. The in-sample RMSE should be close or equal to the out-of-sample RMSE. In this case, our RMSE is \~`{r} format(round(rmse_insample_caret, 0), scientific=FALSE, big.mark=",")` units away from zero.
...
```{r}
#| echo: false
# Convert Python values to R for inline use
max_residual_py_r <- py$max_residual_py
```

Recall that the residual is estimated as the true (observed) value minus the predicted value. Thus: the max(imum) error of `{r} format(round(max_residual_py_r, 0), scientific=FALSE, big.mark=",")` suggests that the model under-predicted expenditure by *this* amount for at least one observation. From the estimation of the predicted residuals we obtain the popular measure of performance evaluation known as the Root Mean Squared Error (RMSE, for short).
...
```{r}
#| echo: false
# Convert Python RMSE to R for inline use
rmse_insample_py_r <- py$rmse_insample_py
```

The RMSE (`{r} format(round(rmse_insample_py_r, 0), scientific=FALSE, big.mark=",")`) gives us an absolute number that indicates how much our predicted values deviate from the true (observed) number. This is all in reference to the target variable. Think of the question: How far, on average, are the residuals away from zero? Generally speaking, the lower the value, the better the model fit. Besides being a good measure of goodness of fit, the RMSE is also useful for comparing the ability of our model to make predictions on different (e.g. test) data sets. The in-sample RMSE should be close or equal to the out-of-sample RMSE. In this case, our RMSE is \~`{r} format(round(rmse_insample_py_r, 0), scientific=FALSE, big.mark=",")` units away from zero.
:::
...
```{r}
#| echo: false
# Convert Python R-squared to R for inline use
r2_train_py_r <- py$r2_train
```

The estimated (Multiple) R-squared of `{r} round(r2_train_py_r, 3)` tells us that our model predicts around `{r} round(r2_train_py_r * 100, 1)` percent of the variation in the target variable (consumption per capita). Also note that when we have a large number of predictors, it's best to look at the Adjusted R-squared, which corrects or adjusts for this by only increasing when a new feature improves the model more so than what would be expected by chance.
:::
...
Please complete all tasks and submit your responses [here](https://classroom.google.com/c/NzAwMjkzOTA5MTM5/a/NzgxMDQ3OTc4OTA2/details) (make sure to join the class first using the link we have sent you). Once submitted, your work will be matched with another course participant for peer review. Please use the blog if you have questions that you can’t resolve or for general feedback. 
...
Using the Session 1 case study, make one small improvement to the baseline model — for example, add or transform a variable, adjust hyperparameters, or test a new model.
Report your specification, performance indicators, and a short explanation of what you did and what changed.
...
<script>
    ...
    this.page.identifier = "session_1.html"; // Replace PAGE_IDENTIFIER with your page's unique identifier variable
    };
